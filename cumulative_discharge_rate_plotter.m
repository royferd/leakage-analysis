...
%     sim_filenames = dir('*discharge-rate-pos*');
    ...
    hv_plot_xy_errors(sprintf('%s 5\\sigma discharge rates',electrodes),...
        'simulation time (hr)','discharges per hour',...
        2,'',1,2,fullpath,sprintf('%s-discharge-rates-sigma-%d',electrodes,i),...
        [0 35 0 500],1:number_rows,zeros(length(1:number_rows),1),...
        dph_sigma(:,1),dph_sigma(:,2),this_set_indices);
    
    hv_plot_xy_errors(sprintf('%s 5\\sigma discharge sizes',electrodes),...
        'simulation time (hr)','median discharge size (pA)',...
        2,'',1,2,fullpath,sprintf('%s-median-discharges-sigma-%d',electrodes,i),...
        [0 35 0 800],1:number_rows,zeros(length(1:number_rows),1),...
        median_sigma,zeros(length(median_sigma),1),this_set_indices);
    
    hv_plot_xy_errors(sprintf('%s 100 pA cutoff discharge rates',electrodes),...
        'simulation time (hr)','discharges per hour',...
        2,'',1,2,fullpath,sprintf('%s-discharge-rates-cutoff-%d',electrodes,i),...
        [0 35 0 500],1:number_rows,zeros(length(1:number_rows),1),...
        dph_cutoff(:,1),dph_cutoff(:,2),this_set_indices);
    
    hv_plot_xy_errors(sprintf('%s 100 pA cutoff discharge sizes',electrodes),...
        'simulation time (hr)','median discharge size (pA)',...
        2,'',1,2,fullpath,sprintf('%s-median-discharges-cutoff-%d',electrodes,i),...
        [0 35 0 800],1:number_rows,zeros(length(1:number_rows),1),...
