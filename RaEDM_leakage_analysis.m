...
[ramp_up_avg_discharge_times,ramp_up_avg_discharge_times_cutoff,...
    ...
    ramp_up_avg_time_length_of_chunk] = ...
    find_discharges_final(lcm1_avg_ramp_up_raw,...
    lcm1_weight_avg_ramp_up_raw,time_ramp_up,lcm1_avg_ramp_up_avg_chunk,...
    lcm1_avg_ramp_up_stdev_chunk,num_up_chunks,...
...
[ramp_up_stdev_discharge_times,ramp_up_stdev_discharge_times_cutoff, ramp_up_stdev_discharge_vals, ...
    ...
    lcm1_weight_avg_ramp_up_raw,time_ramp_up,lcm1_ramp_up_avg_stdev_of_chunk,...
    lcm1_avg_ramp_up_stdev_stdev_chunk*ones(1,num_up_chunks),num_ramp_up_chunks,...
    up_chunk_array,stdev_binwidth,3,abs(lcm1_avg_scale),1,discharge_cutoff,sprintf('stdevs at %.0f kV chunk ',...
    vmon_avg_ramp_up_avg),...
    'neg_voltage_stdev_discharges',1,fullpath);
...
hv_plot_xy_errors('All avg. discharges',...
    ...
    sum(ramp_up_avg_count_discharges(1,:)),sum(ramp_down_avg_count_discharges(1,:)), ...
    sum(zero_avg_count_discharges(1,:))),...
...
hv_plot_xy_errors('All stdev discharges',...
    sprintf('%d discharges at -20 kV \n %d discharges at +20 kV \n %d discharges at 0 kV',...
    sum(ramp_up_stdev_count_discharges(1,:)),sum(ramp_down_stdev_count_discharges(1,:)),...
    sum(zero_stdev_count_discharges(1,:))),...
...
% +/- V stdev discharges
hv_plot_xy_errors('+V/-V stdev discharges',...
    sprintf('%d discharges at -20 kV \n %d discharges at +20 kV \n %d discharges at 0 kV',...
    sum(ramp_up_stdev_count_discharges(1,:)),sum(ramp_down_stdev_count_discharges(1,:)),...
    sum(zero_stdev_count_discharges(1,:))),...
...
hv_plot_xy_errors('+V/-V stdev median discharge size per hour with separate chunks','',...
...
hv_plot_xy_errors('ramp simulation voltage data with trash','',...
1,0,fullpath,'voltage-chunks-color-coded-trash',...
...
hv_plot_xy_errors('ramp simulation leakage data with trash','',...
1,0,fullpath,'leakage-chunks-color-coded-trash',...
time_ramp_down(down_chunk_array(:,1,1)+1:down_chunk_array(:,2,num_down_chunks)),...
zeros(1,length(time_ramp_down(down_chunk_array(:,1,1)+1:down_chunk_array(:,2,num_down_chunks)))),...
...
lcm1_avg_trash_stdev_raw(trash_chunk_array(1,1,1)+1:trash_chunk_array(1,2,num_trash_chunks))*abs(lcm1_avg_scale));
...
hv_plot_xy_errors('ramp simulation zero leakage plotted over all','',...
1,0,fullpath,'leakage-chunks-zero-over-all',...
time,...
zeros(1,length(time)),...
lcm1_avg,...
lcm1_avg_stdev,...
...
hv_plot_xy_errors('ramp simulation up voltage plotted over all','',...
1,0,fullpath,'voltage-chunks-up-over-all',...
time,...
zeros(1,length(time)),...
vmon_avg,...
vmon_avg_stdev,...
