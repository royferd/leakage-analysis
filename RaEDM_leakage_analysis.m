...
datafile ='2019-04-26-151416-hv-1.txt'; power_supply=4; % 29 kV Ti13 run
...
for i = 1:num_files
    ...
    for j = 1:numpoints(i)
        ...
        imon_avg(i,j) = imon_avg_raw(i,j)*polarity_sign(i,j)*imon_avg_scale;
...
[imon_avg_trash_raw,imon_weight_avg_trash_raw,...
    imon_avg_trash_stdev_raw,time_trash,num_trash_points,...
    num_trash_chunks,imon_avg_ramp_down_raw,...
    imon_weight_avg_ramp_down_raw,imon_avg_ramp_down_stdev_raw,...
    time_ramp_down,num_ramp_down_points,num_ramp_down_chunks,...
    imon_avg_ramp_up_raw,imon_weight_avg_ramp_up_raw,...
    imon_avg_ramp_up_stdev_raw,time_ramp_up,num_ramp_up_points,...
    num_ramp_up_chunks,up_chunk_array,...
    down_chunk_array,trash_chunk_array,index_up_chunk_data,index_trash_chunk_data]...
    = sort_state(vmon_avg,vmon_weight,vmon_avg_wt,vmon_weight_raw,...
    start_point,end_point,imon_avg_raw,imon_weight_raw,time,num_rows,...
    numpoints,sampling_time,EDM_sim,power_supply);
...
[imon_avg_charge_neg_raw, imon_stdev_charge_neg_raw,...
    imon_weight_charge_neg_raw, imon_charge_neg_time,imon_avg_charge_pos_raw,...
    imon_stdev_charge_pos_raw,imon_weight_charge_pos_raw,...
    imon_charge_pos_time,imon_avg_discharge_pos_raw,...
    imon_stdev_discharge_pos_raw,imon_weight_discharge_pos_raw,...
    imon_discharge_pos_time,imon_avg_discharge_neg_raw,...
    imon_stdev_discharge_neg_raw,imon_weight_discharge_neg_raw,...
    imon_discharge_neg_time,max_length_discharge,max_length_charge,...
    charge_pos_index,charge_neg_index,discharge_pos_index,...
    discharge_neg_index,discharge_index,charge_index,imon_avg_zero_raw,...
    imon_stdev_zero_raw,imon_weight_avg_zero_raw,...
    zero_time,num_zero_chunks,num_zero_points,zero_chunk_array] = ...
    find_charging_data(num_trash_chunks,trash_chunk_array,lcm1_avg_trash_stdev,...
    lcm1_avg_trash,imon_avg_trash_raw,...
    imon_avg_trash_stdev_raw,...
    imon_weight_avg_trash_raw,time_trash);
...
if EDM_sim ==1
    ...
    [imon_avg_ramp_up_avg_chunk, imon_avg_ramp_up_stdev_chunk,...
        imon_avg_ramp_up_avg,imon_avg_ramp_up_total_stdev,...
        imon_avg_ramp_up_stdev_avg_chunk,imon_avg_ramp_up_stdev_stdev_chunk,...
         imon_ramp_up_stdev_of_stdev_of_chunk,imon_ramp_up_avg_stdev_of_chunk] ...
        = chunkify(num_files,num_up_chunks,...
        num_ramp_up_points,up_chunk_array,imon_avg_ramp_up_raw,...
        imon_weight_avg_ramp_up_raw,imon_avg_scale);
    ...
    [imon_avg_ramp_down_avg_chunk, imon_avg_ramp_down_stdev_chunk,...
        imon_avg_ramp_down_avg,imon_avg_ramp_down_total_stdev,...
        imon_avg_ramp_down_stdev_avg_chunk,imon_avg_ramp_down_stdev_stdev_chunk,...
        imon_ramp_down_stdev_of_stdev_of_chunk,imon_ramp_down_avg_stdev_of_chunk] ...
        = chunkify(num_files,num_down_chunks,...
        num_ramp_down_points,down_chunk_array,imon_avg_ramp_down_raw,...
        imon_weight_avg_ramp_down_raw,imon_avg_scale);
    ...
    [imon_avg_zero_avg_chunk, imon_avg_zero_stdev_chunk,...
        imon_avg_zero_avg,imon_avg_zero_total_stdev,...
        imon_avg_zero_stdev_avg_chunk,imon_avg_zero_stdev_stdev_chunk,...
        imon_zero_stdev_of_stdev_of_chunk,imon_zero_avg_stdev_of_chunk] ...
        = chunkify(num_files,num_zero_chunks,...
        num_zero_points,zero_chunk_array,imon_avg_zero_raw,...
        imon_weight_avg_zero_raw,imon_avg_scale);
...
[zero_avg_discharge_times_sum,zero_avg_discharge_times_sum_cutoff,...
    ...
    one_big_zero_chunk_array,avg_binwidth,3,lcm1_avg_scale,2,discharge_cutoff,sprintf('means at %.0f kV chunk ',...
    vmon_avg_zero_avg),...
    'zero_voltage_avg_steady_state_summed',1,fullpath);
...
max_avg_steady_state = 50*ceil(max([ramp_up_avg_gaus_avg ramp_down_avg_gaus_avg zero_avg_gaus_avg]/50));
min_avg_steady_state = 50*floor(min([ramp_up_avg_gaus_avg ramp_down_avg_gaus_avg zero_avg_gaus_avg]/50));
last_hour_end = 5*ceil(zero_avg_start_stop_times(end,2)/5);
last_hour_start = last_hour_end - 40;
last_twenty_start = last_hour_end - 20;
last_twenty_bounds = [last_twenty_start,last_hour_end,min_avg_steady_state,max_avg_steady_state];
last_hour_bounds = [last_hour_start,last_hour_end,min_avg_steady_state,max_avg_steady_state];
full_bounds = [0,last_hour_end,min_avg_steady_state,max_avg_steady_state];

hv_plot_xy_errors(sprintf('steady-state leakage current'),'time (min)','leakage current (pA)',...
    1,'',1,legend_names(1:3),[],...
    2,fullpath,'steady-state-leakage-vs-time-all',full_bounds,...
    ramp_up_avg_start_stop_times(:,1),zeros(1,length(ramp_up_avg_start_stop_times(:,1))),...
    ramp_up_avg_gaus_avg,zeros(1,length(ramp_up_avg_gaus_avg)),...
    ramp_down_avg_start_stop_times(:,1),zeros(1,length(ramp_down_avg_start_stop_times)),...
    ramp_down_avg_gaus_avg,zeros(1,length(ramp_up_avg_gaus_avg)),...
    zero_avg_start_stop_times,zeros(1,length(zero_avg_start_stop_times(:,1))),...
    zero_avg_gaus_avg,zeros(1,length(zero_avg_gaus_avg)));
    %ramp_down_avg_gaus_avg,ramp_down_avg_gaus_stdev);

hv_plot_xy_errors('EDMRS raw leakage data','time (min)','leakage current (pA)',1,'',...
    1,legend_names,[],1,fullpath,'leakage-chunks-color-coded',full_bounds,...
    ...
    zeros(1,length(lcm1_avg_ramp_up_stdev_raw(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks))*abs(lcm1_avg_scale))),...
    ...
    zeros(1,length(lcm1_avg_ramp_down_stdev_raw(down_chunk_array(:,1,1)+1:down_chunk_array(:,2,num_down_chunks))*abs(lcm1_avg_scale))),...
    ...
    zeros(1,length(lcm1_stdev_zero_raw(zero_chunk_array(1,1,1)+1:zero_chunk_array(1,2,num_zero_chunks))*abs(lcm1_avg_scale))));

hv_plot_xy_errors(sprintf('averaged steady-state leakage current'),'time (min)','leakage current (pA)',...
    1,'',1,legend_names(1:3),[],...
    2,fullpath,'steady-state-leakage-vs-time-all-last-hour',last_hour_bounds,...
    ramp_up_avg_start_stop_times(:,1),zeros(1,length(ramp_up_avg_start_stop_times(:,1))),...
    ramp_up_avg_gaus_avg,zeros(1,length(ramp_up_avg_gaus_avg)),...
    ramp_down_avg_start_stop_times(:,1),zeros(1,length(ramp_down_avg_start_stop_times)),...
    ramp_down_avg_gaus_avg,zeros(1,length(ramp_up_avg_gaus_avg)),...
...
hv_plot_xy_errors('raw steady-state leakage current','time (min)','leakage current (pA)',1,'',...
    1,legend_names,[],1,fullpath,'leakage-chunks-color-coded-last-hour',last_hour_bounds,...
    time_ramp_up(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks)),...
    zeros(1,length(time_ramp_up(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks)))),...
    lcm1_avg_ramp_up_raw(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks))*lcm1_avg_scale,...
    zeros(1,length(lcm1_avg_ramp_up_stdev_raw(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks))*abs(lcm1_avg_scale))),...
...
hv_plot_xy_errors(sprintf('averaged steady-state leakage current'),'time (min)','leakage current (pA)',...
    1,'',1,legend_names(1:3),[],...
    2,fullpath,'steady-state-leakage-vs-time-all-last-few',last_twenty_bounds,...
    ramp_up_avg_start_stop_times(:,1),zeros(1,length(ramp_up_avg_start_stop_times(:,1))),...
    ramp_up_avg_gaus_avg,zeros(1,length(ramp_up_avg_gaus_avg)),...
    ramp_down_avg_start_stop_times(:,1),zeros(1,length(ramp_down_avg_start_stop_times)),...
    ramp_down_avg_gaus_avg,zeros(1,length(ramp_up_avg_gaus_avg)),...
...
hv_plot_xy_errors('raw steady-state leakage current','time (min)','leakage current (pA)',1,'',...
    1,legend_names,[],1,fullpath,'leakage-chunks-color-coded-last-few',last_twenty_bounds,...
    time_ramp_up(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks)),...
    zeros(1,length(time_ramp_up(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks)))),...
    lcm1_avg_ramp_up_raw(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks))*lcm1_avg_scale,...
    zeros(1,length(lcm1_avg_ramp_up_stdev_raw(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks))*abs(lcm1_avg_scale))),...
...
% hv_plot_xy_errors('EDMRS raw leakage data','x axis','y axis',1,'',...
%     1,legend_names,[],1,fullpath,'leakage-chunks-color-coded',last_hour_bounds,...
%     time_ramp_up(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks)),...
%     zeros(1,length(time_ramp_up(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks)))),...
%     lcm1_avg_ramp_up_raw(up_chunk_array(:,1,1)+1:up_chunk_array(:,2,num_up_chunks))*lcm1_avg_scale,...
...
% hv_plot_xy_errors('HVPS current','time (min)','current monitor (uA)',1,'',...
%     0,{'all data','trash','ramp up','ramp down'},[time(end)-60,time(end),-25,25],1,fullpath,'last-few-current-chunks',[],...
%     time(1,last_few_index:end),...
%     zeros(1,length(time(1,last_few_index:end))),...
%     imon_avg(1,last_few_index:end),...
%     imon_avg_stdev(1,last_few_index:end));

hv_plot_xy_errors('HVPS current','time (min)','current monitor (uA)',1,'',...
    0,{'all data','trash','ramp up','ramp down'},[],0,fullpath,...
    'last-few-current-chunks',[time(end)-30,time(end),-25,25],...
    time,...
    zeros(1,length(time)),...
    imon_avg,...
    zeros(1,length(time)));

hv_plot_xy_errors('HVPS signals','time (min)','signal',1,'',...
    1,{'I_{mon} (uA)','V_{mon}/10 (kV)','polarity (V)'},[],1,fullpath,...
    'last-few-chunks',[official_sim_end_time-16,official_sim_end_time,-10,10],...
    time,...
    zeros(1,length(time)),...
    imon_avg,...
    imon_avg_stdev,...
...
fprintf(fileID,'current monitor I_MON = %.2f +/- %.2f uA\n',imon_avg_ramp_up_avg,imon_avg_ramp_up_total_stdev);
